(function (global, factory) {
  ...
  function Helper () {
    ...
    Helper.utils.format = function (config, smallNumber) {
      if (!config) {
        // Default config
        var linearAxis = vm._config.yAxis && vm._config.yAxis.scale === 'linear' ? 'yAxis' : vm._config.xAxis && vm._config.xAxis.scale === 'linear' ? 'xAxis' : '';
        config = {
    ...
    Helper.utils.VirtualScroller = function () {
      ...
      function virtualscroller(container) {
        ...
        function scrollRenderFrame(scrollPosition) {
          ...
          container.each(function () {
            var rowSelection = container.selectAll('.legend-checkbox').data(data.slice(position0, Math.min(position1, totalRows)), dataid);
            rowSelection.exit().call(exit).remove();
            rowSelection.enter().append('g').attr('class', 'legend-checkbox legend').attr('random', function (d) {
              return d;
            }).call(enter).attr('transform', function (d, i) {
      ...
      function drawScrollLegend(visibleRows) {
        ...
        scrollLegend.append('tspan').attr('x', 30).attr('y', -12).text('Desplázate con el');
  ...
  function Chart(config) {
    ...
    Chart.drawLegend = function () {
      ...
      legendBox = vm._fullSvg.append('g').attr('class', 'legendBox').attr('transform', 'translate(' + legendX + ', ' + marginTop + ')').attr('width', vm._width).attr('height', vm._height);
      ...
      if (vm._config.legendTitle) {
        ...
        while (word = words.pop()) {
          ...
          if (tspan.node().getComputedTextLength() > lWidth) {
            ...
            line = [word];
            ++lineNumber;
            tspan = text.append('tspan').attr('x', -6).attr('y', y).attr('dy', lineHeight + 'em').text(word);
          }
        }
      }
      ...
      if (vm._config.legendType === 'checkbox') {
        ...
        var legendEnter = function legendEnter(legendCheck) {
          legendCheck.append('rect').attr('width', size).attr('height', size).attr('x', x).attr('rx', rx).attr('ry', ry).attr('fill-opacity', 1);
          var coordinates = [{
            x: x + size / 8,
            y: _y + size / 2
          }, {
          ...
          legendCheck.append('text').attr('class', 'labelText').attr('x', 20).attr('y', 9).attr('dy', '.35em').attr('text-anchor', 'start');
    ...
    Chart.drawAxes = function () {
      ...
      if (!vm._config.yAxis || vm._config.yAxis && vm._config.yAxis.enabled !== false) {
        ...
        if (vm._config.yAxis && vm._config.yAxis.scaleText) {
          yAxis.append('text').attr('class', 'axis-scale-title').attr('y', -12).attr('x', -12).attr('font-weight', 'bold').attr('text-anchor', 'middle').text(vm._config.yAxis.scaleText);
        }
      }
      ...
      if (vm._config.xAxis && vm._config.xAxis.text) {
        vm._svg.select('.x.axis').append('text').attr('class', 'axis-title').attr('x', vm._width / 2).attr('y', 40).style('text-anchor', 'middle').text(vm._config.xAxis.text);
      }

      if (vm._config.xAxis && vm._config.xAxis.scaleText && vm._xAxis) {
        vm._xAxis.append('text').attr('class', 'axis-scale-title').attr('x', vm._width).attr('y', 15).style('text-anchor', 'start').text(vm._config.xAxis.scaleText);
      }
      ...
      if (!vm._config.xAxis || vm._config.xAxis && vm._config.xAxis.enabled !== false) {
        ...
        var xBandWidth = (vm._scales.x.bandwidth ? vm._scales.x.bandwidth() : (vm._config.size.width - (vm._config.size.margin.left + vm._config.size.margin.right)) / vm._scales.x.ticks()) - 5;
  ...
  function bars (config, helper) {
    ...
    Bars.scales = function () {
      ...
      if (!vm._scales.color || vm._scales.color && vm._scales.color.domain().length === 0) {
        ...
        if (vm._config.legend && vm._config.legend.length > 0) {
          vm._scales.color.domain(vm._config.legend.map(function (d) {
            return d.name;
          }));
        }
        return vm;
