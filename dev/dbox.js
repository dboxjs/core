(function (global, factory) {
...
}(this, (function (exports,cartodb,d3,_$1) { 'use strict';
  ...
  function bars (config, helper) {
    ...
    Bars.init = function (config) {
      ...
      vm._data = [];
      vm._scales = {};
      vm._tip = d3.tip().attr('class', 'd3-tip').direction('n').html(vm._config.tip || function (d) {
        var html = '';
    ...
    Bars._drawGroupByXAxis = function () {
      ...
      vm.chart.svg().append('g').selectAll('g').data(vm._data).enter().append('g').attr('transform', function (d) {
    ...
    Bars._drawStackByXAxis = function () {
      ...
      vm.chart.svg().append('g').selectAll('g').data(vm._data).enter().append('g').attr('fill', function (d) {
        return vm._scales.color(d.key);
      })
          ...
          chart.boxPlots.prepareBoxPlot = function () {
              ...
              for (cName in chart.groupObjs) {
                  ...
                  if (bOpts.showMedian) {
                      ...
                      cBoxPlot.objs.median.circle = cBoxPlot.objs.g.append("circle").attr("class", "median").attr('r', bOpts.medianCSize).style("fill", chart.boxPlots.colorFunct(cName));
                  }
                  ...
                  if (bOpts.showMean) {
                      ...
                      cBoxPlot.objs.mean.circle = cBoxPlot.objs.g.append("circle").attr("class", "mean").attr('r', bOpts.medianCSize).style("fill", chart.boxPlots.colorFunct(cName));
                  }
  ...
  function heatmap (config, helper) {
    ...
    Heatmap.drawColorLegend = function () {
      ...
      var linearGradient = defs.append('linearGradient').attr('id', 'linear-gradient-label');
      ...
      var colorScale = d3.scaleLinear().range(vm._config.colors);

      linearGradient.selectAll('stop').data(colorScale.range()).enter().append('stop').attr('offset', function (d, i) {
      ...
      var legend = vm.chart.svg().append('g').attr('class', 'legend').attr('transform', 'translate(' + (vm._config.size.width - vm._config.size.margin.right + 5) + ',' + vm._config.size.height * .1 + ')');

      //legend title
      legend.append('text').attr('x', 0).attr('class', 'legend-title').attr('text-anchor', 'middle').text(vm._config.legendTitle);

      //top text is the max value
      legend.append('text').attr('x', 0).attr('y', '1.5em').attr('class', 'top-label').attr('text-anchor', 'middle').text(function () {
        var max = Math.ceil(Math.max.apply(Math, toConsumableArray(vm._config.fillValues)));
        return vm.utils.format(max);
      });

      //draw gradient
      legend.append('rect').attr('x', -18).attr('y', '2.3em').attr('width', 18).attr('height', vm._config.size.height * 0.6).attr('fill', 'url(#linear-gradient-label)');

      //bottom text is the min value
      legend.append('text').attr('x', 0).attr('y', vm._config.size.height * 0.6 + 40).attr('class', 'bottom-label').attr('text-anchor', 'middle').text(function () {
        var min = Math.floor(Math.min.apply(Math, toConsumableArray(vm._config.fillValues)));
    ...
    Heatmap.draw = function () {
      ...
      var axesTip = d3.tip().html(function (d) {
        return '<div class="title-tip">' + d + '</div>';
      });
  ...
  function leaflet (config, helper) {
      ...
      if (Array.isArray(objects)) {
        for (var idx = 0; idx < objects.length; idx++) {
          ...
          vm._topojson.objects[obj].geometries.forEach(function (geom) {
            ...
            if (found) geom.properties[vm._config.fill] = found[vm._config.fill];
            vm._nodes.push(geom);
          });
        }
      } else if (objects) {
    ...
    Leaflet.drawColorLegend = function () {
      ...
      //Add gradient legend 
      //defaults to right position
      var legend = d3.select('#' + vm._config.bindTo).select('svg.leaflet-zoom-animated').append('g').attr('class', 'legend').attr('transform', 'translate(' + (vm._config.size.width - 300) + ',' + vm._config.size.height * .1 + ')');
      ...
      //top text is the max value
      legend.append('text').attr('x', 0).attr('y', '1.5em').attr('class', 'top-label').attr('text-anchor', 'middle').text(function () {
        var max = Math.ceil(vm._minMax[1]);
      ...
      //draw gradient
      legend.append('rect').attr('x', -9).attr('y', '2.3em').attr('width', 18).attr('height', vm._config.size.height * 0.6).attr('fill', 'url(#linear-gradient-label)');

      //bottom text is the min value
      legend.append('text').attr('x', 0).attr('y', vm._config.size.height * 0.6 + 40).attr('class', 'bottom-label').attr('text-anchor', 'middle').text(function () {
        var min = Math.floor(Math.min(vm._minMax[0]));
        return vm.utils.format(min);
      });
    };
  ...
  function scatter (config, helper) {
    ...
    Scatter.scales = function () {
      ...
      if (vm._config.hasOwnProperty('colors')) vm._scales.color = d3.scaleOrdinal(vm._config.colors);else vm._scales.color = d3.scaleOrdinal(d3.schemeCategory20c);
      ...
      vm._scales.magnitude = d3.scaleLinear().range(vm._config.magnitudeRange != undefined ? vm._config.magnitudeRange : [5, 20]).domain(magnitudeMinMax).nice();
  ...
  function spineplot (config, helper) {
    ...
    Spineplot.draw = function () {
      ...
      var axesTip = d3.tip().attr('class', 'title-tip').html(function (d) {
      ...
      if (vm._config.orientation === 'horizontal') {
        ...
        vm._xLabels.each(function (d) {
          ...
          var labelMaxWidth = (vm._scales.x(d.x1) - vm._scales.x(d.x0)) * 0.9;
          if (currentWidth < labelMaxWidth * 2) {
            ...
            if (this.getComputedTextLength() > labelMaxWidth) {
              (function () {
                d3.select(_this).on('mouseover', axesTip.show).on('mouseout', axesTip.hide);
                var i = 1;
                while (_this.getComputedTextLength() > labelMaxWidth) {
                  d3.select(_this).text(function (d) {
        ...
        }).attr('height', vm.chart.height).attr('fill', function (d) {
          return vm._scales.color !== false ? vm._scales.color(d[vm._config.fill]) : vm._getQuantileColor(d[vm._config.fill], 'default');
    ...
    Spineplot._drawStackByXAxis = function () {
      ...
      var groups = vm.chart.svg().append('g').selectAll('g').data(vm._data).enter().append('g');
