...
export default function () {
  ...
  Helper.utils.format = function (d, smallVersion, decimals) {
    ...
    if (!fullNumber) {
      var linearAxis = vm._config.yAxis && vm._config.yAxis.scale === 'linear' && _.inRange(d, vm._scales.y.domain()[0], vm._scales.y.domain()[1]) ? 'yAxis' : vm._config.xAxis && vm._config.xAxis.scale === 'linear' && _.inRange(d, vm._scales.x.domain()[0], vm._scales.x.domain()[1]) ? 'xAxis' : '';
      ...
      if (d >= 1000000000000) {
        ...
        d = d / 1000000000000;
      } else if (d >= 1000000000) { // Thousands of millions
        if (linearAxis) {
