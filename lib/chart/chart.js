...
export default function(config) {
  ...
  function Chart(config){
    ...
    vm._addStyle = config.addStyle ? config.addStyle : false;
  ...
  Chart.prototype.addStyle = function(theme){
    var vm = this;
    vm._addStyle = theme;
    return vm;
  };
  ...
  Chart.prototype.drawAxes = function(){
    ...
    if(vm._config.xAxis && vm._config.xAxis.dropdown && vm._config.xAxis.dropdown.enabled === true){
      var xAxisDropDown = d3.select(vm._config.bindTo).append("div").attr('class','dbox-xAxis-select')
                            .append("select")
                            .on("change", function(){
                              vm.updateAxis('dbox-xAxis-select', 'x', this.value);
                            });
      ...
      if(vm._config.xAxis.dropdown.styles){
        ...
        d3.select(".dbox-xAxis-select select option").styles({"text-align": "left"});

      } else {
        d3.select(".dbox-xAxis-select select").styles({"display": "block", "margin": "auto", "text-align": "center", "text-align-last": "center"});
    ...
    if(vm._config.yAxis && vm._config.yAxis.dropdown && vm._config.yAxis.dropdown.enabled === true){
      var yAxisDropDown = d3.select(vm._config.bindTo).append("div").attr('class','dbox-yAxis-select')
                            .append("select")
                            .on("change", function(){
                              vm.updateAxis('dbox-yAxis-select', 'y', this.value);
                            });
      ...
      */
      ...
      var x = -1*d3.select(vm._config.bindTo).node().getBoundingClientRect().width/2+ vm._margin.left/4;
      var y = -1*d3.select(vm._config.bindTo).node().getBoundingClientRect().height/2;
      ...
      if(vm._config.yAxis.dropdown.styles){
        ...
        d3.select(".dbox-yAxis-select select option").styles({"text-align": "left"});

      } else {
        d3.select(".dbox-yAxis-select select").styles({"display": "block", 'transform': 'translate('+x+'px,'+y+'px) rotate(-90deg)', "margin": "auto", "text-align": "center", "text-align-last": "center"});
  ...
  Chart.prototype.updateAxis = function(div, axis, value){
    var vm = this;

    if(div = "dbox-xAxis-select") {
      perpendicular = "dbox-yAxis-select";
    } else if (div = "dbox-yAxis-select") {
      perpendicular = "dbox-xAxis-select";
    }

    var property = d3.select("."+perpendicular+" select option").property('selected');
    ...
    setTimeout(function(){
      d3.select("."+div+" select option").property('selected', null);
      d3.select("."+div+" select option."+value).property("selected", true);
      d3.select("."+perpendicular+" select option").property('selected', null);
      d3.select("."+perpendicular+" select option."+property).property("selected", true);
    }, 40);

    /*
    var q = d3.queue();
    q.defer(layer._chart.draw);
