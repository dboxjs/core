...
export default function (config) {
  ...
  //User
  /**
   * Set newer configuration parameters
  ...
  Chart.legend = function (legend) {
   ...
   /**
   * Set newer configuration parameters
   * @param {String} legendType - Indicates the legend type for legend options
   */
  ...
  Chart.drawLegend = function () {
    ...
    if (isIE || isEdge) {
      var legendX = vm._config.size.width - vm._config.size.margin.right + 10;
      var marginTop = vm._config.size.margin.top + 10;
    ...
    } else if (vm._config.styles && vm._addStyle.legend.position === 'bottom') {
      legendBox = vm._fullSvg
        .append('foreignObject')
        .attr('x', vm._config.size.margin.left)
        .attr('y', vm._config.size.height)
        .attr('width', vm._width)
        .attr('height', vm._height)
        .append('xhtml:div')
        .style('width', vm._width + 'px')
        .style('height', vm._height + 'px')
      ...
      legendBox = vm._fullSvg
        .append('foreignObject')
        .attr('x', vm._config.size.width - vm._config.size.margin.right + 10)
        .attr('y', 0)
        .attr('width', 180)
        .attr('height', vm._config.size.height - 30)
        .append('xhtml:div')
        .style('width', '180px')
        .style('height', (vm._config.size.height - 40) + 'px')
        .style('overflow-y', 'scroll')
        .style('overflow-x', 'hidden')
        .append('svg')
        .attr('width', '180px')
        .attr('height', d3.max([(vm._config.legend.length * 22 + 45), (vm._config.size.height - 30)]) - 20)
        .append('g')
        .attr('class', 'legendBox')
  ...
  Chart.drawAxes = function () {
    ...
    /** Remove ticks showed in xAxis if one of these cases is true:
     * Domain and ticks are disabled in configuration object.
     * Ticks area enabled, but domain is disabled.
     * Ticks configuration is empty (false), and domain is enabled.
     */
    ...
    if (vm._config.xAxis && vm._config.xAxis.dropdown && vm._config.xAxis.dropdown.enabled === true) {
      ...
      //Apply style cofiguration (if is not empty) to xAxis dropdowns
  ...
  /**
   * Update x-axis or y-axis
   * @param {String} axis - Indicates the the selected axis to update, 'x' updates x-axis and 'y' updates y-axis.
   * Falta value
   */
