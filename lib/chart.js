  ...
  Chart.drawLegend = function () {
    ...
    if (isIE || isEdge) {
      ...
      legendBox = vm._fullSvg
        ...
        .append('xhtml:div')
        .style('width', '180px')
        .style('height', (vm._config.size.height - 30) + 'px')
        .style('overflow-y', 'scroll')
        .style('overflow-x', 'hidden')
        .append('svg')
        .attr('width', '180px')
        .attr('height', d3.max([(vm._config.legend.length * 22 + 45), (vm._config.size.height - 30)]))
        .append('g')
        .attr('class', 'legendBox')
      ...
      if (biggestLabelWidth > xBandWidth) { // Biggest label doesn't fit
        vm._xAxis.selectAll('text').each(function (d) {
          if (typeof (d) === 'string') {
            ...
            // Still doesn't fit!
            // Update, before this change vm._config.size.margin.bottom was multiplied by 0.8 
            if (this.getComputedTextLength() > vm._config.size.margin.bottom) {
              d3.select(this)
                .on('mouseover', axesTip.show)
                .on('mouseout', axesTip.hide);
              let i = 1;
              while (this.getComputedTextLength() > vm._config.size.margin.bottom) {
                d3.select(this).text(function (d) {
                  return d.slice(0, -i) + '...';
                }).attr('title', d);
                ++i;
