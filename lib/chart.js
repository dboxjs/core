...
export default function (config) {
  ...
  Chart.drawLegend = function () {
    ...
    while (word = words.pop()) {
      ...
      if (tspan.node().getComputedTextLength() > legendBox.node().getBoundingClientRect().width * 0.7) {
        line.pop();
        tspan.text(line.join(' '));
        line = [word];
        ++lineNumber;
    ...
    if ( vm._config.legendType === 'checkbox') {
      ...
      var legendCheck = legendBox.selectAll('.legend-checkbox')
        ...
        .attr('transform', function(d, i) {return 'translate('+5 +',' + (lineNumber > 1 ? lineNumber * lineHeight + i : 1 + i ) * 19 + ')'; })
      ...
      legendCheck.append("rect")
        ...
        let mark = legendCheck.append("path")
          ...
          .attr("opacity",  function(){ 
            if (d3.select(this).property("checked")) { return 1;
            } else { return 0;} });
        ...
        legendCheck.append('text')
          ...
          //wrap legend title if text size exceeds 70% of container
        var labeltext = legendCheck.selectAll('text').each(function(d){
          if(typeof(d.name) === 'string'){
            if(this.getComputedTextLength() > 0.8 * legendBox.node().getBoundingClientRect().width){
              console.log('label teeeeeeext', d, this.getComputedTextLength(), 0.8 * legendBox.node().getBoundingClientRect().width);
              let i = 1;
              while(this.getComputedTextLength() > 0.8 * legendBox.node().getBoundingClientRect().width){
                d3.select(this).text(function(d){
                  console.log('sliceeeee', d , this, d[name]);
      ...
      if (vm._width / xAxis.selectAll('text').size() > 60) {
        xAxis.selectAll('text').each(function(d){
          if(typeof(d) === 'string') {
            ...
            d3.select(this).call(wrap, xBandWidth * 0.85);
          }
        });
      } else {
        xAxis.selectAll('text').each(function(d){
          d3.select(this).attr('transform', 'translate(-8,' + this.getComputedTextLength() * 0.60 +')rotate(-90)');
          if(typeof(d) === 'string') {
              // vertical labels
              if(this.getComputedTextLength() > 0.8 * vm._margin.bottom) {
                let i = 1;
                while(this.getComputedTextLength() > 0.8 * vm._margin.bottom) {
                  d3.select(this).text(function(d){
                    return d.slice(0,-i) + '...';
                  }).attr('title', d);
                  ++i;
