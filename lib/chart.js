  ...
  Chart.drawLegend = function () {
    ...
    if (vm._config.legendType === 'checkbox') {
      ...
      /* function disableScroll() {
        if (d3.select('.empty-chart').addEventListener) { // older FF
          }
      ...
      /* function ie8SafePreventEvent(e){
        if(d3.select('.empty-chart').addEventListener){ 
          console.log('preventDefault');
          console.log(e.cancelable);
          let emptyChart = d3.select('empty-chart'); 
          emptyChart.addEventListener('DOMMouseScroll', preventDefault, false);
      ...
      d3.select('.empty-chart').on(support, function() {
        var evt = d3.event;
        if(isFirefox) {
          //console.log(support);     
          //console.log('Es Firefox');
          /* d3.select('body').on('wheel', function() {
            let evtBody = d3.event;
            evtBody.preventDefault();
          }); */
          evt.preventDefault();
        } else {
      ...
      d3.select('.arrow-scroll').on(support, function() {
        var evt = d3.event;
        evt.preventDefault();
